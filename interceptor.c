...
void set_addr_rw(unsigned long addr) { // need to find the address of the system call table

	unsigned int level;
	pte_t *pte = lookup_address(addr, &level); //page table is the data structure to map the virtual
...
asmlinkage long my_exit_group(struct pt_regs reg){
	...
	//removes exiting process's pid from all lists
	del_pid(current->pid);
	
	//unlock my_table
	spin_unlock(&my_table_lock);
	
	//calls the original exit_group
...
asmlinkage long interceptor(struct pt_regs reg) {
	long unsigned int system_call_number = reg.ax;
	
	//lock my_table
	spin_lock(&my_table_lock);
	
	//when all pids are being mornitored for this syscall we can just log the message
	if(table[system_call_number].monitored == 2) {
		log_message(current->pid, system_call_number, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	} 

	//when some pids are being monitored and the current pid is being monitored log the message
	if (table[system_call_number].monitored == 1) {
		if (check_pid_monitored(system_call_number, current->pid)) {
			log_message(current->pid, system_call_number, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		}
	} 
	...
	return table[system_call_number].f(reg);
}
 ...
 *     a) Cannot de-intercept a system call that has not been intercepted yet. (DONE)
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	//Error Condition A: syscall number must be valid
    	if ((syscall < 0) || (syscall > NR_syscalls) || (syscall == MY_CUSTOM_SYSCALL)) {
        	return -EINVAL;
    	}

    	//Error Condition A: pid must be valid for monitoring commands
	...
	//Error Condition B: check all four commands for right permissions
	//Must be root user for REQUEST_SYSCALL_INTERCEPT and REQUEST_SYSCALL_RELEASE.
	//Checks if pid is zero or the pid is not from the calling process and we are not root.
	if ((((cmd == REQUEST_SYSCALL_INTERCEPT) || (cmd == REQUEST_SYSCALL_RELEASE)) && current_uid() != 0) || (((cmd == REQUEST_START_MONITORING) || (cmd == REQUEST_STOP_MONITORING)) &&((current_uid() != 0) && ((pid == 0) || (check_pids_same_owner(current->pid, pid) != 0))))) {
		return -EPERM;
	}
	...
	//Error Condition D: intercepting a system call that is already intercepted
	...
	//Error Condition C: cannot de-intercept a system call that has not been intercepted yet
       	if ((table[syscall].intercepted == 0)&&((cmd == REQUEST_SYSCALL_RELEASE) || (cmd == REQUEST_STOP_MONITORING) || (cmd == REQUEST_START_MONITORING))) { 
        	//unlock my_table
		spin_unlock(&my_table_lock);
		return -EINVAL;
    	}

	//Error Condition D: monitoring a pid that is already being monitored
	if (cmd == REQUEST_START_MONITORING && pid == 0 && table[syscall].monitored == 2 && table[syscall].listcount == 0) {
                //unlock my_table
		spin_unlock(&my_table_lock);
		return -EBUSY;
        }

	// Error Condition C: Cannot monitor a pid that is already being monitored and where all pids have already been set to being monitored 
        if (cmd == REQUEST_START_MONITORING && check_pid_monitored(syscall, pid) == 1 && table[syscall].monitored == 2) {
                //unlock my_table
		spin_unlock(&my_table_lock);
		return -EINVAL; 
	}

	// Error Conditions for REQUEST_START_MONITORING
    	if (cmd == REQUEST_START_MONITORING) {
		// Error Condition D: Cannot monitor a pid that is already being monitored and where not all pids are monitored, or where a pid is not checked but all pids are being monitored. 
        	if ((((check_pid_monitored(syscall, pid) == 1) && (table[syscall].monitored != 2)) || ((check_pid_monitored(syscall, pid) == 0) && (table[syscall].monitored == 2)))) {
            		//unlock my_table
			spin_unlock(&my_table_lock);
	...
	//Error Condition C: cannot stop monitoring for a pid that is not being monitored
	if (cmd == REQUEST_STOP_MONITORING && pid == 0 && table[syscall].monitored == 0) {
                //unlock my_table
		spin_unlock(&my_table_lock);
	...
	// Error Condition C: Cannot stop monitoring a pid that is not monitored yet. 
        if (cmd == REQUEST_STOP_MONITORING && check_pid_monitored(syscall, pid) == 0 && table[syscall].monitored != 2) {
                //unlock my_table
		spin_unlock(&my_table_lock);
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		...
		//original system call will be saved
		table[syscall].intercepted = 1;
		table[syscall].f = sys_call_table[syscall];

		//entry in the sys_call_table will be replaced with a generic interceptor function
		set_addr_rw((unsigned long) sys_call_table);
		sys_call_table[syscall] = interceptor;
		set_addr_ro((unsigned long) sys_call_table);

		//unlock my_table and sys_call_table
		spin_unlock(&my_table_lock);
	...
	//Release Command
		...
		//the original saved system call is restored in the sys_call_table
		set_addr_rw((unsigned long) sys_call_table);
	...
	//Start Monitoring Command
		...
		// Start monitoring all pids
		if (pid == 0) {
			//When pid is zero, clear the list of all previously tracked pids since now we are tracking them all
			destroy_list(syscall);
			// Set monitored to two as all pids are being monitored
			table[syscall].monitored = 2;
			
			//unlock my_table
			spin_unlock(&my_table_lock);
		}// Start monitoring one pid
		else{
			//Add pid to syscall's list of monitored pids 
			if(add_pid_sysc(pid, syscall) != 0) {
				
				//unlock my_table
				spin_unlock(&my_table_lock);
				// Error Condition E: Not enough memory available
				return -ENOMEM;
			}
			//Only some or one pids are being monitored
	...
	//Stop Monitoring Command
		...
		if (pid == 0) {
			//When pid is zero, clear the syscalls list of all monitored pids
                        destroy_list(syscall);
			// Monitored is zero since all pids are now not being monitored
                        table[syscall].monitored = 0;

			//unlock my_table
                        spin_unlock(&my_table_lock);
                
		} else { // Stop monitoring one pid
			//Delete the pid from the syscalls list of monitored pids
			if (del_pid_sysc(pid, syscall) != 0) {
				//unlock my_table
				spin_unlock(&my_table_lock);
				return -EINVAL;
                        }
			//If the listcount is zero then there are no more pids being monitored in that syscall
			//Therefore, set monitored to zero
		...
		//Error Condition A: invalid command
		return -EINVAL;
	}
	return 0;
}
...
static void exit_function(void){
	...
	for (i = 0; i < NR_syscalls; i++) {
		if (table[i].intercepted) {
			my_syscall(REQUEST_SYSCALL_RELEASE, i, 0); //deintercepts the i syscall
			table[i].f = NULL;
		}
		table[i].intercepted = 0;
		table[i].monitored = 0;
